...
function CustomItemModal({ currency, onAdd, onClose }: {
  currency: string;
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              onKeyDown={e => e.key === "Enter" && valid && (() => { onAdd(name.trim(), Math.round(parseFloat(price) * 100)); })()}
          ...
          <button
            onClick={() => onAdd(name.trim(), Math.round(parseFloat(price) * 100))}
            disabled={!valid}
...
function TerminalPickerModal({ terminals, selected, onSelect, onClose }: {
            ...
            {terminals.map(t => (
              <button
                ...
                className={`w-full flex items-center gap-4 px-5 py-4 rounded-xl border-2 transition-all text-left ${
                  selected === t.terminalId ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-gray-300"
                }`}
              >
                ...
                {selected === t.terminalId && <CheckCircle size={18} className="ml-auto text-green-500" />}
              </button>
            ))}
          </div>
        )}
...
function SaleCompleteOverlay({ sale, currency, onDismiss }: {
  ...
  return (
    <div
      className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-green-600 cursor-pointer select-none"
      onClick={onDismiss}
    >
      <div className="flex flex-col items-center gap-4 text-white px-8 text-center">
        ...
        {sale.method !== "terminal"
          ? <p className="text-4xl font-black tracking-tight">{formatCurrency(sale.total, currency)}</p>
          : <p className="text-2xl font-bold">Sent to terminal</p>
        }
        <p className="text-green-200 text-sm font-medium">
          {methodLabel}{sale.memberName ? ` · ${sale.memberName}` : ""}
        </p>
...
function Clock() {
  ...
  return <span className="text-sm font-mono text-gray-400">{time}</span>;
}

// ── Main operational POS ──────────────────────────────────────────────────────
...
export default function POSOperatePage() {
  ...
  const selectedMember  = useMemo(() => members?.find(m => m._id === selectedMemberId) ?? null, [members, selectedMemberId]);
  ...
  function addToBasket(item: { id: Id<"posProducts"> | "custom"; name: string; pricePence: number }) {
    setBasket(prev => {
      ...
      return [...prev, { productId: item.id, productName: item.name, quantity: 1, unitPricePence: item.pricePence, subtotalPence: item.pricePence }];
    });
  }

  function changeQty(productId: Id<"posProducts"> | "custom", delta: number) {
    setBasket(prev => prev
  ...
  function clearAll() {
    setBasket([]); setNote(""); setShowNote(false);
    setSelectedMemberId(null); setMemberSearch(""); setSaleDone(null);
  }
  ...
  async function handleCharge() {
    ...
    try {
      const items = basket.map(i => ({
        ...
        unitPricePence: i.unitPricePence, subtotalPence: i.subtotalPence,
      }));
      ...
      if (paymentMethod === "terminal") {
        ...
        await recordSale({ clubId: club._id, memberId: selectedMember?.userId, memberName: selectedMember?.displayName, items, currency, paymentMethod: "terminal", notes: note || undefined });
      } else if (paymentMethod === "account") {
        await recordSale({ clubId: club._id, chargeAccountMemberId: selectedMemberId!, items, currency, paymentMethod: "account", notes: note || undefined });
      } else {
        await recordSale({ clubId: club._id, memberId: selectedMember?.userId, memberName: selectedMember?.displayName, items, currency, paymentMethod, notes: note || undefined });
      }

      setSaleDone({ total, method: paymentMethod, memberName: selectedMember?.displayName });
      setBasket([]); setNote(""); setShowNote(false); setSelectedMemberId(null); setMemberSearch("");
    } catch (err: unknown) {
    ...
    <div className="flex h-screen overflow-hidden bg-gray-100 select-none">
      ...
      <div className="w-[72px] shrink-0 flex flex-col bg-white border-r border-gray-200 overflow-y-auto py-2 gap-1">
        ...
        {isAdmin && (
          <Link
            href="/manage/pos"
            className="mx-1.5 mb-1 flex flex-col items-center gap-1 py-3 px-1 rounded-xl text-gray-400 hover:bg-gray-100 transition-colors"
            title="Back to management"
          >
            <ArrowLeft size={18} />
            <span className="text-[9px] font-medium">Manage</span>
          </Link>
        )}
        {/* Divider if admin */}
          ...
          return (
            <button
              ...
              className={`mx-1.5 flex flex-col items-center gap-1 py-3 px-1 rounded-xl text-center transition-all ${
                active ? "bg-green-600 text-white shadow-sm" : "text-gray-500 hover:bg-gray-100"
              }`}
            >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header — minimal, just club name + clock */}
        <div className="flex items-center justify-between px-5 py-2.5 bg-white border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-400">Point of Sale</span>
          </div>
          <Clock />
        </div>
                ...
                return (
                  <button
                    ...
                    className={`relative flex flex-col justify-between p-3.5 rounded-2xl border-2 min-h-[90px] text-left transition-all active:scale-95 ${
                      outOfStock ? "border-gray-100 bg-white opacity-40 cursor-not-allowed"
                      : inBasket  ? "border-green-500 bg-green-50 shadow-sm"
      ...
      <div className="w-80 shrink-0 flex flex-col bg-white border-l border-gray-200 relative">
        ...
        {saleDone && <SaleCompleteOverlay sale={saleDone} currency={currency} onDismiss={clearAll} />}
              ...
              {showMemberSearch && (
                <div className="absolute top-full left-0 right-0 mt-1 z-20 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                  <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100">
                    ...
                    <input autoFocus value={memberSearch} onChange={e => setMemberSearch(e.target.value)} placeholder="Search member…" className="flex-1 text-sm outline-none" />
                    <button onClick={() => { setShowMemberSearch(false); setMemberSearch(""); }}><X size={14} className="text-gray-400" /></button>
                  </div>
                  <div className="max-h-48 overflow-y-auto">
                    {filteredMembers.map(m => (
                      <button key={m._id} onClick={() => { setSelectedMemberId(m._id); setShowMemberSearch(false); setMemberSearch(""); }} className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-gray-50 text-left">
                    ...
                    {filteredMembers.length === 0 && <p className="px-4 py-3 text-sm text-gray-400">No members found</p>}
                  </div>
                </div>
              )}
            </div>
              ...
              {basket.map((item, idx) => (
                <div key={`${item.productId}-${idx}`} className="flex items-center gap-2 px-4 py-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => changeQty(item.productId, -1)} className="w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 text-gray-600 hover:bg-red-50 hover:text-red-500 active:scale-90 transition-all">
                      <Minus size={12} />
                    </button>
                    <span className="text-sm font-bold text-gray-900 w-6 text-center">{item.quantity}</span>
                    <button onClick={() => changeQty(item.productId, 1)} className="w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 text-gray-600 hover:bg-green-50 hover:text-green-600 active:scale-90 transition-all">
                      <Plus size={12} />
                    </button>
                  </div>
                  <span className="text-sm font-bold text-gray-900 w-14 text-right shrink-0">{formatCurrency(item.subtotalPence, currency)}</span>
                  <button onClick={() => setBasket(prev => prev.filter((_, i) => i !== idx))} className="text-gray-200 hover:text-red-400 transition-colors ml-1"><X size={13} /></button>
                </div>
              ))}
            </div>
          )}
        ...
        {basket.length > 0 && (
          <div className="px-4 pb-1">
            {showNote ? (
              <div className="flex items-center gap-2">
                <input autoFocus value={note} onChange={e => setNote(e.target.value)} placeholder="Add a note…" className="flex-1 text-xs border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-green-500" />
                <button onClick={() => { setShowNote(false); setNote(""); }} className="text-gray-300 hover:text-gray-500"><X size={13} /></button>
              </div>
            ) : (
              <button onClick={() => setShowNote(true)} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-600 transition-colors py-1">
        ...
        <div className="border-t border-gray-100 p-4 space-y-3">
          <div className="flex items-baseline justify-between px-1">
            <span className="text-sm text-gray-500">{itemCount > 0 ? `${itemCount} item${itemCount !== 1 ? "s" : ""}` : "Total"}</span>
            <span className="text-4xl font-black text-gray-900 tracking-tight">{formatCurrency(total, currency)}</span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-3 rounded-xl border-2 font-semibold text-sm transition-all active:scale-95 ${
                    active ? `${m.colour} border-current` : "border-gray-200 text-gray-600 bg-white hover:border-gray-300"
                  }`}
                >
                  <Icon size={16} />
                  ...
                  {m.id === "terminal" && active && selectedTerminalId && (
                    <button onClick={e => { e.stopPropagation(); setShowTerminalPicker(true); }} className="ml-auto text-current opacity-60 hover:opacity-100">
                      <Settings size={12} />
                    </button>
                  )}
                </button>
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-2 rounded-xl border text-sm font-medium transition-all ${
              paymentMethod === "complimentary" ? "border-gray-400 bg-gray-100 text-gray-700" : "border-gray-200 text-gray-400 hover:text-gray-600 hover:border-gray-300"
            }`}
          >
          ...
          >
            {saving ? "Processing…"
              ...
              : paymentMethod === "terminal" ? "Send to terminal"
              : `Charge ${formatCurrency(total, currency)}`}
          </button>

          {basket.length > 0 && (
            <button onClick={clearAll} className="w-full py-1.5 text-xs text-gray-400 hover:text-gray-600 transition-colors">
              Clear basket
            </button>
          )}
        </div>
      ...
      {showCustomItem && (
        <CustomItemModal
          currency={currency}
          onAdd={(name, pricePence) => { addToBasket({ id: "custom", name, pricePence }); setShowCustomItem(false); }}
